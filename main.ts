// =====================================================================
// MR SHREY API - Deno Deploy (Fast & Stable)
// =====================================================================
...
async function fastFetch(url: string, options: any = {}, timeout: number = 8000) {
...
async function handler(req: Request): Promise<Response> {
  ...
  if (parts[0] === "rootx" && parts.length === 3) {
    ...
    if (!typeMap[type]) {
      return new Response(JSON.stringify({
        status: "error",
        message: "Invalid type. Use: vehicle, number, aadhar",
        developer: "MR SHREY",
        channel: "https://t.me/MR_SHREY3"
      }), { status: 400, headers });
    }
    ...
    try {
      const response = await fastFetch(
        `https://rootx-osint.in/?type=${typeMap[type]}&key=seed_bhai&query=${query}`,
...
console.log("🚀 MR SHREY API Gateway Running!");
