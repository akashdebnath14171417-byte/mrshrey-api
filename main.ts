...
function getKeyInfo(apiKey: string) {
  ...
  if (!data) return null;
  const daysLeft = Math.ceil((new Date(data.expiry).getTime() - Date.now()) / (1000*60*60*24));
  return {
    plan: data.plan,
    expiry: data.expiry,
    days_left: daysLeft,
...
function validateApiKey(apiKey: string) {
  ...
  if (new Date() > new Date(data.expiry)) return { valid: false, error: "❌ API Key Expired!" };
  const today = new Date().toISOString().split('T')[0];
  if (data.last_reset !== today) { data.used_today = 0; data.last_reset = today; }
  ...
  async fetch(request: Request): Promise<Response> {
    ...
    if (parts.length === 0 || (parts.length === 1 && parts[0] === "")) {
      return new Response(JSON.stringify({
        ...
        version: "2.0",
        endpoints: {
          "/pan/<pan>": { example: "/pan/JCZPS4827P?api_key=MR_SHREY_MONTHLY_001" },
          "/vehicle91/<rc>": { example: "/vehicle91/MH12DE1433" },
    ...
    if (parts[0] === "vehicle91" && parts.length === 2) {
      const rc = parts[1].toUpperCase().replace(/[\s-]/g, '');
      if (!/^[A-Z]{2}\d{1,2}[A-Z]{1,3}\d{1,4}$/.test(rc)) {
        return new Response(JSON.stringify({
          status: "error",
      ...
      try {
        const resp = await fetch("https://api1.91wheels.com/api/v1/third/rc-detail", {
          method: "POST",
          headers: { "Content-Type": "application/json", "Accept": "application/json" },
          body: JSON.stringify({ regNo: rc, sessionid: crypto.randomUUID() + "-" + crypto.randomUUID() })
        });
    ...
    if (parts[0] === "pan" && parts.length === 2) {
      ...
      if (!apiKey) {
        return new Response(JSON.stringify({
          ...
          developer: "MR SHREY", channel: "https://t.me/MR_SHREY3"
        }, null, 2), { status: 400, headers });
      }
      ...
      try {
        const resp = await fetch(
          ...
          { headers: { "x-broker": "turtlemint", "authorization": "Bearer f13517d5a59b689d16aa30c528ccaf7801f823b0f5548f65d6d3793270cfe8d628cea877289aba166e5425c31cfc7a0b", "x-provider": "signzy" } }
        );
        const data = await resp.json();
